...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   08:58 Wednesday December 19, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
ph:=ph||'			$(''#queuestatus'').text(''Files Selected: ''+numFilesSelected+'' / Queued Files: ''+numFilesQueued);'||chr(10)||
'		})'||chr(10)||
''||chr(10)||
'		.bind(''uploadStart'', function(event, file){'||chr(10)||
''||chr(10)||
'                        var param = {''actualfilename'' : ''&USER_SID.''+file.name};'||chr(10)||
'                        $(this).swfupload(''setPostParams'', param);'||chr(10)||
''||chr(10)||
'			$(''#log li#''+file.id).find(''p.status'').text(''Uploading...'');'||chr(10)||
'			$(''#log li#''+file.id).find(''span';

ph:=ph||'.progressvalue'').text(''0%'');'||chr(10)||
...
'			var percentage=Math.round((bytesLoaded/file.size)*100);'||chr(10)||
...
ph:=ph||'uccess'', function(event, file, serverData){'||chr(10)||
'			var item=$(''#log li#''+file.id);'||chr(10)||
'			item.find(''div.progress'').css(''width'', ''100%'');'||chr(10)||
'			item.find(''span.progressvalue'').text(''100%'');'||chr(10)||
'			item.addClass(''success'').find(''p.status'').html(''Upload complete...Loading into Database...'');'||chr(10)||
''||chr(10)||
'                        var get = new htmldb_Get(null,'||chr(10)||
'                                                 $v(''pFlowId''),'||chr(10)||
'    ';

ph:=ph||'                                             ''APPLICATION_PROCESS=ATTACHMENTS_SAVE_TO_DATABASE'','||chr(10)||
'                                                 $v(''pFlowStepId''));'||chr(10)||
''||chr(10)||
'                        var serverDataSplit = serverData.split("~~~");'||chr(10)||
''||chr(10)||
'                        get.addParam(''x01'',''&P5100_OBJ1.'');        // OBJ'||chr(10)||
'                        get.addParam(''x02'',''&USER_SID.'');          // User SID'||chr(10)||
'        ';

ph:=ph||'                get.addParam(''x03'',serverDataSplit[0]);    // Filename'||chr(10)||
...
'                        else '||chr(10)||
'                          item.find(''p.status'').html(''Upload comple';

ph:=ph||'te...''+gReturn);'||chr(10)||
'		})'||chr(10)||
''||chr(10)||
'		.bind(''uploadError'', function(event, file, errorCode, message){'||chr(10)||
'			var item=$(''#log li#''+file.id);'||chr(10)||
'			item.addClass(''error'').find(''p.status'').html(''Upload Failed...''+message);'||chr(10)||
'		})'||chr(10)||
''||chr(10)||
...
ph:=ph||'getInstance(''#swfupload-control'').getStats();'||chr(10)||
''||chr(10)||
...
'</script>',
  ...
  p_last_upd_yyyymmddhh24miss => '20121219085757',
...
'                          located in T_CORE_CONFIG - OSI.ATTCH_MXSZ.');
