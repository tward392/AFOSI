...
--- CR#4098  ---
----------------
UPDATE T_OSI_TAB SET TAB_LABEL='Surveillance/Intercept Information' WHERE TAB_LABEL='Intercept Information';
COMMIT;

-----------------------------------------
--- CR#4114 Leave Default Title Blank ---
-----------------------------------------
UPDATE T_OSI_OBJ_TYPE SET DEFAULT_TITLE=NULL WHERE SID='22200000MS7';
COMMIT;

-------------------------------------------------
--- CR#4114 Remove OTHER from Equipment Types ---
-------------------------------------------------
UPDATE T_OSI_REFERENCE SET ACTIVE='N' WHERE USAGE='SURV_EQUIP' AND CODE='OTHER';
COMMIT;

-----------------------------------------------------------------------------
--- CR#4114 Added Surveillance Conducted and Surveillance Location Fields ---
-----------------------------------------------------------------------------
ALTER TABLE T_OSI_A_SURVEILLANCE ADD SURVEILLANCECONDUCTED VARCHAR2(1);

INSERT INTO T_OSI_ADDR_TYPE ( SID, OBJ_TYPE, USAGE, CODE, DESCRIPTION, MIN_NUM, MAX_NUM, SEQ, ACTIVE, CREATE_BY, CREATE_ON, MODIFY_BY, MODIFY_ON ) VALUES ( '33319BZA', '22200000MS7', 'ADDR_SURV', 'ADDR_SURV', 'Address of Surveillance', NULL, NULL, 1, 'Y', 'timothy.ward',  TO_Date( '10/16/2012 01:33:51 PM', 'MM/DD/YYYY HH:MI:SS AM'), 'timothy.ward',  TO_Date( '10/16/2012 01:33:51 PM', 'MM/DD/YYYY HH:MI:SS AM')); 
COMMIT;
...
(SID, ACTIVITY_DATE, EMERGENCY, CONSENSUAL, US_LOCATION, 
 ...
 ACTIVATION_DATE, TSO_COORD_DATE, ADDRESS_INTRCPT, ADDRESS_SURV, INTERCEPT_LINE, PWA_REQUIRED, 
 ...
 EQ_LOC_LIST, EQUIPMENT_LIST, PURPOSE_LIST, DEACTIVATION_DATE, SURVEILLANCECONDUCTED)
AS 
select a.sid, a.activity_date, s.emergency, s.consensual, s.us_location, s.item_case,
           ...
           s.tso_coord_date,
           osi_address.get_addr_fields (osi_address.get_address_sid(a.sid, 'ADDR_INTRCPT')) as address_intrcpt,
           osi_address.get_addr_fields (osi_address.get_address_sid(a.sid, 'ADDR_SURV')) as address_surv,
...
begin
    ...
    update t_osi_a_surveillance
       set emergency = nvl(:new.emergency, 'N'),
           ...
           deactivation_date = :new.deactivation_date,
           ...
           surveillanceconducted = :new.surveillanceconducted
     where SID = :new.SID
       and (   nvl(emergency, 'x') <> nvl(:new.emergency, 'x')
            ...
            or nvl(deactivation_date, sysdate) <> nvl(:new.deactivation_date, sysdate)
            ...
            or nvl(surveillanceconducted, 'x') <> nvl(:new.surveillanceconducted, 'x')
    ...
    osi_address.update_single_address(:new.SID, 'ADDR_SURV', :new.address_surv);
