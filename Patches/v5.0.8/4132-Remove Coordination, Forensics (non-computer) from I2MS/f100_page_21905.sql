...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   12:21 Monday October 22, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
wwv_flow_api.create_page(
  ...
  p_last_upd_yyyymmddhh24miss => '20121022121629',
...
'                       changes and save it.'||chr(10)||
''||chr(10)||
'02-Aug-2012 - Tim Ward - P0_OBJ Passing in the Page Branches.'||chr(10)||
''||chr(10)||
'22-Oct-2012 - Tim Ward - CR#4132 - Remove Coordination, Forensics (Non-Computer)'||chr(10)||
'                          from create list.  Noticed double entries for current'||chr(10)||
'                          type (changed union all to union to avoid this).');
...
wwv_flow_api.create_page_branch(
  ...
  p_flow_id=> wwv_flow.g_flow_id,
  p_flow_step_id=> 21905,
  p_branch_action=> 'f?p=&APP_ID.:21905:&SESSION.:&REQUEST.:&DEBUG.::P0_OBJ:&P21905_SID.&success_msg=#SUCCESS_MSG#',
  p_branch_point=> 'AFTER_PROCESSING',
  p_branch_type=> 'REDIRECT_URL',
...
'from V_OSI_CONSULT_CREATE_TYPES WHERE ACTIVE=''Y'''||chr(10)||
'union'||chr(10)||
...
'end;',
  ...
  p_display_as=> 'COMBOBOX',
  p_lov => 'select D display_value, R return_value '||chr(10)||
'from V_OSI_COORDIN_CREATE_TYPES WHERE ACTIVE=''Y'''||chr(10)||
'union'||chr(10)||
'select substr(description, 14, length(description) - 13) display_value, sid return_value'||chr(10)||
'  from t_core_obj_type'||chr(10)||
' where sid=:P0_OBJ_TYPE_SID'||chr(10)||
...
declare
  ...
  l_length number := 1;
begin
p:=p||'P21905_TYPE_CONSULTATION,P21905_TYPE_COORDINATION,P21905_CONTACT_METHOD,P21905_DATE,P21905_HOUR,P21905_MINUTE';

wwv_flow_api.create_page_process(
  p_id     => 15604222895686506 + wwv_flow_api.g_id_offset,
  p_flow_id=> wwv_flow.g_flow_id,
  p_flow_step_id => 21905,
  ...
  p_process_sql_clob => p, 
  p_process_error_message=> '',
  p_process_when_button_id=>1108000073521362 + wwv_flow_api.g_id_offset,
  p_process_success_message=> '',
  p_process_is_stateful_y_n=>'N',
  p_process_comment=>'');
end;
