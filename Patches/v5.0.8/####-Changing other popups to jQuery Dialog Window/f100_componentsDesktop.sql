...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   10:58 Wednesday August 29, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
c1:=c1||'(CheckAccess(pSID)=="Y")'||chr(10)||
...
c1:=c1||'                              if (CheckAccess(pSID)=="Y")'||chr(10)||
''||chr(10)||
...
'                                         var get = new htmldb_Get(null,'||chr(10)||
'                                                                  $v(''pFlowId''),'||chr(10)||
'                                                                  ''APPLICATION_PROCESS=KeepUnkeepRemove_From_R';

c1:=c1||'ecent_Objects'','||chr(10)||
'                                                                  $v(''pFlowStepId''));  '||chr(10)||
''||chr(10)||
'                                         get.addParam(''x01'',pSID);'||chr(10)||
...
c1:=c1||'                                 window.location.reload();'||chr(10)||
'                                         break;'||chr(10)||
''||chr(10)||
'                           case ''Ticket'':'||chr(10)||
...
c1:=c1||'01'',''P5000_PERSONNEL'',''Y'','''',''OPEN'','''','''',''Choose Personnel...'',''PERSONNEL'',''&P0_OBJ.'');'||chr(10)||
...
c1:=c1||'ts = (tag == "*" && elm.all)? elm.all : elm.getElementsByTagName(tag);'||chr(10)||
...
'</script>'||chr(10)||
''||chr(10)||
'<script type="text/javascript" src="#IMAGE_PREFIX#jQuery/jquery-datetimepicker.js"></';
...
c1:=c1||'ript type=''text/javascript'' src=''/i/jquery/tipsy-v1.0.0a-24/src/javascripts/jquery.tipsy.js''></script>'||chr(10)||
...
'		$(''div.tooltip'').tipsy({live: true, fade';

c1:=c1||': true, gravity: ''n'', title: ''tip''});'||chr(10)||
'        });'||chr(10)||
''||chr(10)||
'// Allows Resizing of Report Column Drop-downs //'||chr(10)||
'$(function()'||chr(10)||
'{  '||chr(10)||
' $("#apexir_rollover").resizable({minWidth:$("#apexir_rollover").width(), handles:"e", start:function(e,u){document.body.onclick="";},stop:function(e,u){$(this).css({"height":""}); setTimeout(function(){document.body.onclick=gReport.dialog.check;},100);}}).css({"z-index":9999}).chi';

c1:=c1||'ldren(".ui-resizable-e").css({"background":"#EFEFEF"});'||chr(10)||
'});'||chr(10)||
'</script>'||chr(10)||
...
c1:=c1||'rc="#IMAGE_PREFIX#jQuery/superfish-1.4.8/js/supersubs.js"></script>'||chr(10)||
'<script type="text/javascript">'||chr(10)||
' '||chr(10)||
' $(document).ready(function(){ '||chr(10)||
...
c1:=c1||'                         // due to slight rounding differences and font-family '||chr(10)||
...
c1:=c1||'f="#IMAGE_PREFIX#themes/OSI/i2ms.ico">'||chr(10)||
'</head>'||chr(10)||
