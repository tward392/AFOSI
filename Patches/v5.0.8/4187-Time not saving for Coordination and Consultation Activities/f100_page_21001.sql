...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   10:52 Monday November 19, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
'</script>',
  ...
  p_last_upd_yyyymmddhh24miss => '20121119105250',
...
'19-Nov-2012 - Tim Ward - CR#4187 - Time not saving for COORD/CONSULT Activities.'||chr(10)||
'');
...
p:=p||':p21001_activity_date;'||chr(10)||
''||chr(10)||
'    WHEN :P21001_OBJ_TYPE_CODE=''ACT.INTERVIEW.GROUP'' THEN'||chr(10)||
''||chr(10)||
...
'        v_FieldValues := '''''''' || ''~^~P0_OBJ~^~'' || '''''''' || '','' || '''''''' || repla';

p:=p||'ce(:p21001_list_1,''%null%'',''null'') || '''''''';'||chr(10)||
...
p:=p||'BJ_TYPE_CODE=''ACT.LIAISON'' THEN'||chr(10)||
''||chr(10)||
...
'        v_ActivityDate := to_date(:p21001_a';

p:=p||'ctivity_date, ''DD-MON-YYYY HH24:MI'');'||chr(10)||
''||chr(10)||
...
'        v_FieldValues := ''core_sidgen.next_sid'' || '','' || '''''''' || replace(:p21001_list_1,''%null%'',''null'')';
...
'        v_FieldValues := '''''''' || ''~^~P0_OBJ~^~'' || '''''''' || '','' || '''''''' || replace(:p21001_text_5,'''''''','''''''''''') ||';

p:=p||' '''''''' || '','' || '||chr(10)||
'''to_date(to_char(to_date('' || '''''''' || :p21001_activity_date || '''''''' || ''),''''yyyymmdd'''')'' || '' ||nvl('' || '''''''' || :p21001_hour || '''''''' || '',''''01'''')'' || '' || nvl('' || '''''''' || :p21001_minute || '''''''' || '',''''00'''') || '' || ''''''00'''''' || '', ''''yyyymmddhh24miss'''')''  || '','' || '||chr(10)||
'''to_date(to_char(to_date('' || '''''''' || :p21001_activity_date || '''''''' || ''),''''yyyymmdd'''')'' || '' ||nvl('' || '''''''' || :p2';

p:=p||'1001_hour_2 || '''''''' || '',''''01'''')'' || '' || nvl('' || '''''''' || :p21001_minute_2 || '''''''' || '',''''00'''') || '' || ''''''00'''''' || '', ''''yyyymmddhh24miss'''')''  || '','' || '''''''' ||'||chr(10)||
...
p:=p||'ate(:p21001_activity_date, ''DD-Mon-YYYY HH24:MI'');'||chr(10)||
''||chr(10)||
...
'        v_FieldValues := '''''''' || ''~^~P0_OBJ~^~'' || '''''''' || '','' || '''''''' || replace(:p21001_list_2,''%';

p:=p||'null%'',''null'') || '''''''' || '','' || '''''''' ||'||chr(10)||
...
p:=p||'TYPE_CODE=''ACT.SOURCE_MEET'' THEN'||chr(10)||
''||chr(10)||
...
'        v_ActivityDate := to_date(to_char(to_';

p:=p||'date(:p21001_activity_date),''yyyymmdd'') || nvl(:p21001_hour,''01'') || nvl(:p21001_minute,''00'') || ''00'', ''yyyymmddhh24miss'');'||chr(10)||
''||chr(10)||
...
'        v_FieldValues := '''''''' || ''~^~P0_OBJ~^~'' || '''''''' || '','' || '''''''' || :p21001_radiogroup_2 || '''''''' || ';
...
p:=p||'01_list_2,''%null%'',''null'') || '''''''' || '','' || '''''''' ||'||chr(10)||
...
p:=p||'AddressCode := ''ADDR_INTRCPT'';'||chr(10)||
'      '||chr(10)||
''||chr(10)||
...
p:=p||'           :p21001_participant_version,'||chr(10)||
