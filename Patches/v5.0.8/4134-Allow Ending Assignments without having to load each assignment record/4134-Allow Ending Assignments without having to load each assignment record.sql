...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   08:45 Tuesday October 23, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
'}'||chr(10)||
''||chr(10)||
'function getAssignmentList(pSelectedOrAll)'||chr(10)||
'{'||chr(10)||
' var pSelectedAssignments=''~'';'||chr(10)||
''||chr(10)||
' $(''Input''+pSelectedOrAll).each(function()'||chr(10)||
'  {'||chr(10)||
'   if($(this).attr(''name'')==''f01'')'||chr(10)||
'     {'||chr(10)||
'      pSelectedAssignments+=$(this).attr(''id'')+''~'';'||chr(10)||
'     }'||chr(10)||
'  });'||chr(10)||
''||chr(10)||
' return pSelectedAssignments;'||chr(10)||
'}'||chr(10)||
''||chr(10)||
...
'}'||chr(10)||
''||chr(10)||
'function endAssignments(pSelectedOrAll)'||chr(10)||
'{'||chr(10)||
' var sids='''';'||chr(10)||
''||chr(10)||
' if(pSelectedOrAll==''SELECTED'')'||chr(10)||
'   {'||chr(10)||
'    if(isOneSelected()==false)'||chr(10)||
'      {'||chr(10)||
'       alert(''Please select assignments to end first!'');'||chr(10)||
'       return;'||chr(10)||
'      }'||chr(10)||
'    else'||chr(10)||
'      sids = getAssignmentList('':checked'');'||chr(10)||
'   }'||chr(10)||
' else'||chr(10)||
'   {'||chr(10)||
'    if(confirm(''Are you sure you want to End All Assignments?'')==false)'||chr(10)||
'      return;'||chr(10)||
'   }'||chr(10)||
' '||chr(10)||
' var get = new htmldb_Get(null,'||chr(10)||
'                          $v(''pFlowId''),'||chr(10)||
'                          ''APPLICATION_PROCESS=ASSIGNMENTS_END'','||chr(10)||
'                          $v(''pFlowStepId''));'||chr(10)||
''||chr(10)||
' get.addParam(''x01'',$v(''P5000_OBJ1''));'||chr(10)||
' get.addParam(''x02'',sids);'||chr(10)||
' '||chr(10)||
...
'</script>',
  ...
  p_last_upd_yyyymmddhh24miss => '20121023084534',
...
'                        pass :P0_OBJ.  Created Setup Process.'||chr(10)||
''||chr(10)||
'27-Feb-2012 - Tim Ward - CR#4002 - Changed to JQuery Locators Page #301.'||chr(10)||
'                         Also added links to view Personnel Record (in-line with Legacy I2MS).'||chr(10)||
''||chr(10)||
'23-Oct-2012 - Tim Ward - CR#4134 - All Ending All or Selected Assignments.'||chr(10)||
'');
...
'       ''<input type="checkbox" value="'' || sid || ''" name="f01"'' || '' id="'' || sid || ''">'' as "Select",'||chr(10)||
'       OSI_OBJECT.GET_TAGLINE_LINK(personnel) as "Personnel",'||chr(10)||
...
s := null;
wwv_flow_api.create_report_columns (
  p_id=> 17197628076202920 + wwv_flow_api.g_id_offset,
  p_region_id=> 89951737688285648 + wwv_flow_api.g_id_offset,
  p_flow_id=> wwv_flow.g_flow_id,
  p_query_column_id=> 9,
  p_form_element_id=> null,
...
wwv_flow_api.create_page_button(
  p_id             => 17197801326214099 + wwv_flow_api.g_id_offset,
  p_flow_id        => wwv_flow.g_flow_id,
  p_flow_step_id   => 5000,
  p_button_sequence=> 80,
  p_button_plug_id => 89951737688285648+wwv_flow_api.g_id_offset,
  p_button_name    => 'END_SELECTED_ASSIGNMENTS',
  p_button_image   => 'template:'||to_char(179221063608554498+wwv_flow_api.g_id_offset),
  p_button_image_alt=> 'End Selected Assignments',
  p_button_position=> 'REGION_TEMPLATE_CHANGE',
  p_button_alignment=> 'LEFT',
  p_button_redirect_url=> 'javascript:void(0);',
  p_button_cattributes=>'onClick="javascript:endAssignments(''SELECTED'');"',
  p_required_patch => null + wwv_flow_api.g_id_offset);
 
wwv_flow_api.create_page_button(
  p_id             => 17198416002227802 + wwv_flow_api.g_id_offset,
  p_flow_id        => wwv_flow.g_flow_id,
  p_flow_step_id   => 5000,
  p_button_sequence=> 90,
  p_button_plug_id => 89951737688285648+wwv_flow_api.g_id_offset,
  p_button_name    => 'END_ALL_ASSIGNMENTS',
  p_button_image   => 'template:'||to_char(179221063608554498+wwv_flow_api.g_id_offset),
  p_button_image_alt=> 'End All Assignments',
  p_button_position=> 'REGION_TEMPLATE_CHANGE',
  p_button_alignment=> 'LEFT',
  p_button_redirect_url=> 'javascript:void(0);',
  p_button_cattributes=>'onClick="javascript:endAssignments(''ALL'');"',
  p_required_patch => null + wwv_flow_api.g_id_offset);
...
begin
wwv_flow_api.create_page_item(
  p_id=>11665517928782368 + wwv_flow_api.g_id_offset,
  p_flow_id=> wwv_flow.g_flow_id,
  p_flow_step_id=> 5000,
  p_name=>'P5000_PERSONNEL_VIEW_WIDGET',
  p_data_type=> 'VARCHAR',
  p_accept_processing=> 'REPLACE_EXISTING',
  p_item_sequence=> 21,
  ...
  p_use_cache_before_default=> 'YES',
  p_item_default_type => 'STATIC_TEXT_WITH_SUBSTITUTIONS',
  p_source=>'''<a title="View the Assigned Personnel" href="'' || osi_object.get_object_url(:P5000_PERSONNEL) || ''">&ICON_MAGNIFY.</a>''',
  ...
  p_cHeight=> 5,
  p_cAttributes=> 'nowrap="nowrap"',
  p_tag_attributes  => 'style="vertical-align:bottom; padding-left:0px;"',
  p_begin_on_new_line => 'NO',
  ...
  p_label_alignment  => 'LEFT',
  p_field_alignment  => 'LEFT',
  p_display_when=>':P5000_PERSONNEL is not null',
...
'<a href="javascript:openLocator(''301'',''P5000_PERSONNEL'',''Y'','''',''OPEN'','''','''',''Choose Personnel...'',''PERSONNEL'',''&P0_OBJ.'');" style="vertical-align:bottom; padding-left:2px;">&ICON_LOCATOR.</a>'||chr(10)||
'',
...
'end;',
  ...
  p_cHeight=> 5,
  p_cAttributes=> 'nowrap',
  p_tag_attributes  => 'onFocus="javascript:openLocator(''301'',''P5000_PERSONNEL'',''Y'','''',''OPEN'','''','''',''Choose Personnel...'',''PERSONNEL'',''&P0_OBJ.'');"',
  p_begin_on_new_line => 'YES',
  p_begin_on_new_field=> 'YES',
  p_colspan => 1,
  p_rowspan => 1,
