...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   11:18 Wednesday November 7, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
' else'||chr(10)||
'   runJQueryPopWin(''Complete/Close Selected Activities'',''P5600_ACTION_TYPE,P5600_ACT_LIST,P0_OB';

ph:=ph||'J'',''SEL,''+getActList('':checked'')+'',&P0_OBJ.'',''5600'',600,350);'||chr(10)||
'}'||chr(10)||
''||chr(10)||
...
' else'||chr(10)||
'   runJQueryPopWin(''Complete/Close ALL Activities'',''P5600_ACTION_TYPE,P5600_ACT_LIST';

ph:=ph||',P0_OBJ'',''ALL,''+getActList('''')+'',&P0_OBJ.'',''5600'',600,350);'||chr(10)||
''||chr(10)||
'}'||chr(10)||
''||chr(10)||
...
ph:=ph||' = document.getElementById(pThat);'||chr(10)||
...
ph:=ph||'want to remove the selected associations?''))'||chr(10)||
...
'</script>',
  ...
  p_last_upd_yyyymmddhh24miss => '20121107111801',
...
'                          and they need to auto generate.');
...
s:=s||'date as "Suspense Date", '||chr(10)||
...
'       ''<a href="#" onClick="javascript:runJQueryPopWin(''''IDP Notes'''',''''P5060_OBJ'''','''''' || ACTIVITY_SID || '''''','' || ''''''5060'''');"><img src="#IMAGE_PREFIX#themes/OSI/notes.gif" alt="View IDP Notes"></a>'' as "Notes Pop-Up",'||chr(10)||
'   ';

s:=s||'    osi_util.get_report_menu(activity_sid) as "Reports"'||chr(10)||
...
s:=s||''' || '');" id="SUB_'' || activity_sid || ''" title="Toggle Field">'' || decode(substantive,''Y'',''Yes'',''No'') || ''</a>'',decode(substantive,''Y'',''Yes'',''No'')) as "Substantive",'||chr(10)||
'       decode(leadership_approved,''Y'',''Yes'',''No'') as "Approved",'||chr(10)||
'       activity_id as "Activity ID", '||chr(10)||
'       activity_type_desc as "Type of Activity", '||chr(10)||
'       osi_object.get_tagline_link(activity_sid) as "Activity Title",'||chr(10)||
...
s:=s||'ivity_date as "Activity Date",'||chr(10)||
'       activity_unit_assigned as "Assigned Unit",'||chr(10)||
'       activity_suspense_date as "Suspense Date", '||chr(10)||
'       activity_complete_date as "Completed", '||chr(10)||
'       activity_close_date as "Closed", '||chr(10)||
'       decode(sid, :P10150_SEL_ASSOC, ''Y'', ''N'') as "Current",'||chr(10)||
'       ''<a href="#" onClick="javascript:runJQueryPopWin(''''IDP Notes'''',''''P5060_OBJ'''','''''' || ACTIVITY_SID || '''''','' || ''''';

s:=s||'''5060'''');"><img src="#IMAGE_PREFIX#themes/OSI/notes.gif" alt="View IDP Notes"></a>'' as "Notes Pop-Up",'||chr(10)||
