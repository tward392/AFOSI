CREATE OR REPLACE procedure Update_Support_Objects (pParent in Varchar2, pInfo in Varchar2) is
begin
     --- Support object tables ---
     update T_CFUNDS_EXPENSE_V3 set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_ASSIGNMENT set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_ATTACHMENT set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_WORK_HOURS set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_NOTE set PARENT_INFO = pInfo where PARENT = pParent;
     update T_OSI_NOTIFICATION_EVENT set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_PARTIC_CONTACT set PARENT_INFO = pInfo where PARENT = pParent;
     ----->NO PARENT_INFO FIELD IN WEB<----update T_OSI_PARTIC_INVOLVEMENT set PARENT_INFO = pInfo where PARENT = pParent;

end Update_Support_Objects;
/


CREATE OR REPLACE TRIGGER ACT_UPDATE_SUPPORT_OBJS before update of ID, TITLE on T_OSI_ACTIVITY for each row
begin
     UPDATE_SUPPORT_OBJECTS(:old.SID, 'Activity: ' || :new.ID || ' - ' || :new.TITLE);
end;


CREATE OR REPLACE TRIGGER FILE_UPDATE_SUPPORT_OBJS before update of ID, TITLE on T_OSI_FILE for each row
begin
     UPDATE_SUPPORT_OBJECTS(:old.SID, 'File: ' || :new.ID || ' - ' || :new.TITLE);
end;


CREATE OR REPLACE TRIGGER UNIT_CODE_UPDATE_SUPPORT_OBJS before update of UNIT_CODE on T_OSI_UNIT for each row
begin
     UPDATE_SUPPORT_OBJECTS(:old.SID, :new.unit_code || ' - ' || osi_unit.get_name(:old.sid));
end;


CREATE OR REPLACE TRIGGER UNIT_NAME_UPDATE_SUPPORT_OBJS before insert on T_OSI_UNIT_NAME for each row
begin
     UPDATE_SUPPORT_OBJECTS(:old.SID, osi_unit.get_code(:old.unit) || ' - ' || :new.unit_name);
end;


ALTER TRIGGER UNIT_CODE_UPDATE_SUPPORT_OBJS DISABLE;

ALTER TRIGGER UNIT_NAME_UPDATE_SUPPORT_OBJS DISABLE;
...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   06:49 Tuesday May 10, 2011
--   Exported By:     TWARD
--   Flashback:       0
...
'',
  ...
  p_last_upd_yyyymmddhh24miss => '20110510064915',
...
'                              PARENT_INFO when saving a new Expense.');
