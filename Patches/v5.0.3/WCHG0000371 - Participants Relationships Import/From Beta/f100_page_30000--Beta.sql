...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   13:11 Monday April 4, 2011
--   Exported By:     TWARD
--   Flashback:       0
...
'                    name:''&P5450_CLONE_NEW_SID.'', item_names:''P0';
...
'</script>',
  ...
  p_page_comment  => '<script language="JavaScript">'||chr(10)||
''||chr(10)||
'if("&REQUEST." == "CREATE_RETURN" || "&REQUEST." == "CREATE_UNK_RETURN"){'||chr(10)||
'  passBack("&P30000_SEL_PARTICIPANT.", "&REQUEST.");'||chr(10)||
'}'||chr(10)||
''||chr(10)||
'function passBack(pSid, pRequest){'||chr(10)||
'  opener.document.getElementById("&P30000_RETURN_ITEM.").value = pSid;'||chr(10)||
'  opener.doSubmit(pRequest);'||chr(10)||
'  close();'||chr(10)||
'}'||chr(10)||
'function useSelected(pMessage, pRequest){'||chr(10)||
'  if(confirm(pMessage)){'||chr(10)||
'    passBack("&P30000_SEL_PARTICIPANT.", pRequest);'||chr(10)||
'  }'||chr(10)||
'}'||chr(10)||
'/*'||chr(10)||
' * This function is available to prompt the user. It was being used by button'||chr(10)||
...
p:=p||'       elsif(instr(:p30000_deers_result, ''MSG'') > 0) then'||chr(10)||
...
'            :p30000_deers_msg := ''DEERS match found. Confirmed par';

p:=p||'ticipant was created.'';'||chr(10)||
