...
******************************************************************************/
    ...
    BEGIN
        FOR x IN (SELECT   *
                     ...
                     WHERE (   participant_version = p_pvop
                            or participant_version = osi_participant.get_current_version(p_pvop))
        ...
        LOOP
            ...
            IF (k.name_type IS NOT NULL) THEN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        log_error
                            ('import_legacy_part_version: Error locating description for name_type: '
                             || k.name_type || ' ' || SQLERRM);
                END;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            log_error('osi_participant.import_legacy_part_version: ' || SQLERRM || ' '
                      || dbms_utility.format_error_backtrace || ' using legacy version: '
                      || p_legacy_person_version);
            RAISE;
...
----------------------------
        log_error
            ('import_legacy_relationships: STATUS - fetching the object description for version: '
             || p_person_version);
        ...
        IF (v_temp IS NOT NULL) THEN
            ...
            SELECT SID
             ...
             WHERE obj_type = Core_Obj.lookup_objtype(v_obj_desc)
               AND from_status = Osi_Status.get_starting_status(Core_Obj.lookup_objtype(v_obj_desc))
        ...
        LOOP
            ...
            LOOP
                ...
                begin
                    select osi_personnel.get_name(v_temp)
                      into v_create_by
                      from dual;
                exception
                    when others then
                ...
                update t_osi_attachment
                   set create_by = v_create_by,
                       create_on = j.attach_date
                 where SID = v_new_sid_temp;
            END LOOP;
...
----------------------
    ...
    BEGIN
        ...
        IF p_i2ms_person is null then
            BEGIN
                SELECT old_sid
                  ...
                  FROM T_OSI_MIGRATION
                 WHERE new_sid = p_new_person AND type = 'PARTICIPANT';
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
        ...
        FOR k IN (SELECT *
                    FROM ref_v_person_relation
                   WHERE this_person = v_source_person AND SID not in(SELECT old_sid
                                                                        FROM t_osi_migration
        ...
        LOOP
            ...
            BEGIN
                SELECT new_sid
                  ...
                  FROM t_osi_migration
                 WHERE type = 'PARTICIPANT' AND old_sid = v_rel_person;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_new_rel_person := null;
            END;
            ...
            IF v_new_rel_person is null then
                ...
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        --return an error message
                        return 'Error: Unable to locate a current version in legacy for '
                               || v_rel_person;
                END;
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    --return an error message
                    return 'Error: Unable to locate relationship type for '
                           || nvl(k.rel_desc, 'NULL');
            END;
        ...
        UPDATE T_OSI_MIGRATION
           SET imp_relations = 'Y'
         WHERE type = 'PARTICIPANT' AND new_sid = v_new_source_person AND imp_relations = 'N';
    ...
    BEGIN
        SELECT COUNT(*)
          INTO v_cnt
          FROM T_OSI_PARTICIPANT_VERSION pv, V_OSI_PARTIC_RELATION_2WAY pr
         WHERE pr.this_partic = pv.participant
           AND upper(pr.rel_desc) LIKE '%SPOUSE%'
           AND (   pv.participant = pPoPV
                OR pv.SID = pPoPV)
           AND NVL(pr.END_DATE,(SYSDATE + 1)) > SYSDATE;
...
------------------------
    ...
    BEGIN
        select imp_relations
          into v_return
          from t_osi_migration
         where new_sid = p_obj and type = 'PARTICIPANT';
