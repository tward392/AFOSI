...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   12:01 Tuesday June 14, 2011
--   Exported By:     TWARD
--   Flashback:       0
...
wwv_flow_api.create_page(
  ...
  p_last_upd_yyyymmddhh24miss => '20110614120153',
...
''||chr(10)||
'14-Jun-2011  Tim Ward  CR#3758 - Allow anything > 0 (wasn''t allowing .5).');
...
wwv_flow_api.create_page_branch(
  ...
  p_flow_id=> wwv_flow.g_flow_id,
  p_flow_step_id=> 5250,
  p_branch_action=> 'f?p=&APP_ID.:5250:&SESSION.:&REQUEST.:&DEBUG.:::',
  p_branch_point=> 'AFTER_PROCESSING',
  p_branch_type=> 'REDIRECT_URL',
...
' if v_date > trunc(sysdate) then'||chr(10)||
'   return false;'||chr(10)||
' else'||chr(10)||
'   return true;'||chr(10)||
' end if;'||chr(10)||
...
declare
  ...
  l_length number := 1;
begin
p:=p||'if :p5250_pay_cat_lov is null then'||chr(10)||
