...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   12:52 Wednesday July 11, 2012
--   Exported By:     TWARD
--   Flashback:       0
--   Export Type:     Page Export
--   Version: 3.2.0.00.27
...
' var vOk = true;'||chr(10)||
''||chr(10)||
' if ($v(''P30710_TRANS_TYPE'')==''LOGIN'' && $v(''P30710_EV_STATUS'')==''S'')'||chr(10)||
...
'      javascript:newWindow({page:800,clear_cache:'''',name:'''',item_names:''P800_REPORT_TYPE,P0_OBJ,P800_PARAM_2'',item_values:''&P30710_I2MS_RPT.,&P0_OBJ.,&P30710_NEW_SID_FOR_SPLIT.,'',request:''OPEN''});'||chr(10)||
''||chr(10)||
'    if(''&P30710_AUTO_PRINT_TAG.''==''AFF52'')'||chr(10)||
'      javascript:newWindow({page:800,clear_cache:'''',name:'''',item_names:''P800_REPORT_TYPE,P0_OBJ,P800_PARAM_2'',item_values:''&P30710_ORIG_RPT.,&P0_OBJ.,&P30710_NEW_SID_FOR_SPLIT.,'',request:''OPEN''});'||chr(10)||
'   }'||chr(10)||
' else'||chr(10)||
'   {'||chr(10)||
'    if(''&P30710_AUTO_PRINT_TAG.''==''I2MS'')'||chr(10)||
'    javascript:newWindow({page:800,clear_cache:'''',name:'''',item_names:''P800_REPORT_TYPE,P0_OBJ,P800_PARAM_2'',item_values:''&P30710_I2MS_RPT.,&P0_OBJ.,&P30710_SEL_EVIDENCE.,'',request:''OPEN''});'||chr(10)||
''||chr(10)||
'    if(''&P30710_AUTO_PRINT_TAG.''==''AFF52'')'||chr(10)||
'      javascript:newWindow({page:800,clear_cache:'''',name:'''',item_names:''P800_REPORT_TYPE,P0_OBJ,P800_PARAM_2'',item_values:''&P30710_ORIG_RPT.,&P0_OBJ.,&P30710_SEL_EVIDENCE.,'',request:''OPEN''});'||chr(10)||
'   }'||chr(10)||
'}'||chr(10)||
''||chr(10)||
...
'   AutoPrintTag();'||chr(10)||
''||chr(10)||
' opener.doSubmit(''REFRESH'');'||chr(10)||
' window.close();'||chr(10)||
'}'||chr(10)||
'</script>',
  p_autocomplete_on_off => 'ON',
  ...
  p_required_patch=> null + wwv_flow_api.g_id_offset,
  p_last_updated_by => 'TWARD',
  p_last_upd_yyyymmddhh24miss => '20120711125206',
  p_page_is_public_y_n=> 'N',
  p_protection_level=> 'N',
...
wwv_flow_api.create_page_plug (
  ...
  p_pagination_display_position=>'BOTTOM_RIGHT',
  p_plug_header=> '<a class="htmlButton" href="javascript:window.close();" onclick="mySubmit(''Close'');">Close</a><br>',
  p_plug_customized=>'0',
  p_plug_caching=> 'NOT_CACHED',
  p_plug_comment=> '');
end;
...
p:=p||'login_evidence(:p30710_trans_sids,'||chr(10)||
'                                        :p0_obj,'||chr(10)||
...
p:=p||'T_TAG:=''YES'';'||chr(10)||
''||chr(10)||
'            :P30710_NEW_SID_FOR_SPLIT:=osi_evidence.split_tag_evidence(core_list.get_list_element(:p30710_trans_sids, 1),'||chr(10)||
'                                                                  :p30710_effective_date,'||chr(10)||
'                                                                  :p30710_purpose,'||chr(10)||
'                                                                  :p30710_description,'||chr(10)||
'   ';
...
p:=p||'                                          :p30710_transfer_unit,'||chr(10)||
'                                           :p0_obj,'||chr(10)||
'                                           :p30710_condition_change);'||chr(10)||
...
p:=p||'                         :p30710_effective_date,'||chr(10)||
'                                            :p30710_purpose);'||chr(10)||
'        else'||chr(10)||
'            null;'||chr(10)||
'    end case;'||chr(10)||
...
------------------------
    ...
    function generate_i2ms_tag_rpt(p_obj in varchar2, p_evidence in varchar2) return clob is
        v_ok                 varchar2(100);
        v_template           clob;
        v_template_date      date;
        v_mime_type          t_core_template.mime_type%type;
    ...
    begin
          ...
          for a in (select * from v_osi_evidence_trans_log where evidence_sid = p_evidence order by sort_date asc, modify_on asc)
          loop
              ...
              v_temp_string := v_temp_string || '\fs24\lang1033\langfe1033\cgrid\langnp1033\langfenp1033 {\b\fs20\insrsid13830262 ' || v_record_count || '\cell ' || a.tran_desc || '\cell ' || a.purpose || '\cell ' || to_char(a.effective_date, 'DD-Mon-YYYY') || '\cell ' || a.custodian_name || '\cell \cell }\pard\plain \ql \li0\ri0\widctlpar\intbl\aspalpha\aspnum\faauto\adjustright\rin0\lin0';
    ...
    function generate_orig_tag_rpt(p_obj in varchar2, p_evidence in varchar2) return clob is
        v_ok                 varchar2(100);
        v_template           clob;
        v_template_date      date;
        v_mime_type          t_core_template.mime_type%type;
    ...
    begin
         v_ok := core_template.get_latest('EVIDENCE_TAG_ORIGINAL',v_template,v_template_date,v_mime_type,v_mime_disposition);
          ...
          select count(*) into v_record_count from v_osi_evidence_trans_log where evidence_sid = p_evidence order by sort_date asc, modify_on asc;
          ...
          if v_record_count > 15 then
            ...
            for a in (select custodian_name,effective_date,tran_desc,decode(condition_change,'Y','Yes','No') as condition_change,other_party
                         from v_osi_evidence_trans_log where evidence_sid = p_evidence order by sort_date desc, modify_on desc)
            loop
            ...
            for a in (select custodian_name,effective_date,tran_desc,decode(condition_change,'Y','Yes','No') as condition_change,other_party
                            from v_osi_evidence_trans_log where evidence_sid=p_evidence order by sort_date asc,modify_on asc)
            loop
                v_ok := core_template.replace_tag(v_template,'CCRBY' || ltrim(rtrim(to_char(v_record_count))),osi_report.replace_special_chars(a.custodian_name || ', ' || to_char(a.effective_date,v_date_format),htmlorrtf));
                v_ok := core_template.replace_tag(v_template,'CCPUR' || ltrim(rtrim(to_char(v_record_count))),osi_report.replace_special_chars(a.tran_desc,htmlorrtf));
                v_ok := core_template.replace_tag(v_template,'CCCON' || ltrim(rtrim(to_char(v_record_count))),osi_report.replace_special_chars(a.condition_change,htmlorrtf));
