...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   11:04 Thursday May 24, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
'<link rel="stylesheet" type="text/css" href="#IM';

c1:=c1||'AGE_PREFIX#jquery/css/redmond1.8.20/jquery-ui-1.8.20.custom.css" />'||chr(10)||
''||chr(10)||
...
'	var testClass = new RegExp("(^|\\s)" + className ';
...
c1:=c1||'icker to all DatePicker input fields not hidden //'||chr(10)||
...
'          {'||chr(10)||
'           inputs[i].className="datepickerdisabl';
...
'  $(''a.pickToday'').click(function() {$(this';

c1:=c1||').prev().prev(''input.datepickernew'').datepicker(''setDate'', new Date());});'||chr(10)||
' });'||chr(10)||
'</script>'||chr(10)||
''||chr(10)||
...
'<script src="#IMAGE_PREFIX#jquery/cluetip-1.2.2/jquery.c';

c1:=c1||'luetip.js" type="text/javascript"></script>'||chr(10)||
' '||chr(10)||
'<script type="text/javascript">'||chr(10)||
'//<![CDATA['||chr(10)||
' '||chr(10)||
...
'      $(''#apexir_ACTIONSMENU a[tit';

c1:=c1||'le="Help"]'').prop({''href'':''&HELP_URL.'',''target'':''_blank''});'||chr(10)||
'   });'||chr(10)||
' '||chr(10)||
'//]]>'||chr(10)||
'</script>'||chr(10)||
