RENAME T_OSI_A_RECORDS_CHECK_MULTI TO T_OSI_A_RC_DR_MULTI_TEMP;

ALTER TABLE T_OSI_A_RC_DR_MULTI_TEMP RENAME COLUMN REFERENCE_NUMBER TO REFERENCE_NUM;

ALTER TABLE T_OSI_A_RC_DR_MULTI_TEMP ADD OBJ_TYPE_CODE VARCHAR2(100);

RENAME T_OSI_A_RECORDS_CHECK_RESULTS TO T_OSI_A_RC_DR_RESULTS;

ALTER TABLE T_OSI_A_RC_DR_MULTI_TEMP ADD EXPLANATION VARCHAR2(1000);
...
******************************************************************************/
    ...
    begin
         ...
         for k in (select a.SID, a.id as act_no, a.title, a.activity_date, a.narrative, a.object_type_code 
...
alter table t_osi_a_document_review
  add obj varchar2(20);

ALTER TABLE t_osi_a_document_review DROP CONSTRAINT FK_OSI_DOCREV_OBJ; 

ALTER TABLE WEBI2MS.T_OSI_A_DOCUMENT_REVIEW ADD (
  CONSTRAINT FK_OSI_DOCREV_OBJ 
 FOREIGN KEY (OBJ) 
 REFERENCES WEBI2MS.T_CORE_OBJ (SID)
    ON DELETE CASCADE);
...
update t_osi_a_document_review set obj=sid;
commit;



CREATE OR REPLACE TRIGGER WEBI2MS.t_osi_a_doc_rev_b_i_sid
    before insert on t_osi_a_document_review for each row
begin
    if :new.SID is null then
       :new.SID := Core_Sidgen.NEXT_SID;
    end if;
...
update t_osi_a_document_review rc
  set narrative=(select narrative from t_osi_activity a where rc.sid=a.sid);
commit;

---update t_osi_activity set narrative=null where sid in (select sid from t_osi_a_document_review);
---commit;


alter table t_osi_a_document_review add activity_date date; 


update t_osi_a_document_review dr
  set activity_date=(select activity_date from t_osi_activity a where dr.sid=a.sid);
commit;


alter table t_osi_a_document_review add results varchar2(10);


ALTER TABLE t_osi_a_document_review  ADD (
  CONSTRAINT FK_OSI_A_DR_RESULTS 
 FOREIGN KEY (RESULTS) 
 REFERENCES T_OSI_A_RC_DR_RESULTS (CODE));
...
alter table t_osi_a_document_review add create_by varchar2(100);
alter table t_osi_a_document_review add create_on date;

alter table t_osi_a_document_review add modify_by varchar2(100);
alter table t_osi_a_document_review add modify_on date;

CREATE OR REPLACE TRIGGER WEBI2MS."OSI_DR_B_IU_TS" 
    before insert or update
    on t_osi_a_document_review
    for each row
/*
    This is a custom trigger. It is not generated like all the other _TS triggers.
...
update t_osi_a_document_review dr
  set create_by=(select create_by from t_core_obj a where dr.sid=a.sid)
  where dr.create_by is null;
commit;

update t_osi_a_document_review dr
  set create_on=(select create_on from t_core_obj a where dr.sid=a.sid)
  where dr.create_on is null;
commit;

update t_osi_a_document_review dr
  set modify_by=(select modify_by from t_core_obj a where dr.sid=a.sid)
  where dr.modify_by is null;
commit;

update t_osi_a_document_review dr
  set modify_on=(select modify_on from t_core_obj a where dr.sid=a.sid)
  where dr.modify_on is null;
commit;
...
alter table t_osi_a_document_review modify (obj not null);
...
UPDATE T_OSI_TAB SET PAGE_NUM='21505' WHERE SID='222000008QQ';
COMMIT;
