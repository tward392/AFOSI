 ...
 * @author - Richard Norman Dibble
 /******************************************************************************
   ...
   27-Oct-2011  Tim Ward        CR#3963 - Include Associated Files and Inherited Activities IDP Notes report in IDP NOTES REPORT.
...
function rpt_generate_add_info_sections(
    ...
    begin
        ...
        for k in (select obj, note_text, seq, 'Note from:  ' || core_obj.get_parentinfo(n.obj) as parentinfo
                        from t_osi_note n, t_osi_note_type nt
                        where n.note_type=nt.sid 
                          and   (n.obj=p_obj 
                             or (n.obj in (select that_file from v_osi_assoc_fle_fle where this_file=p_obj)) 
                             or (n.obj in (select activity_sid from v_osi_assoc_fle_act where file_sid=p_obj)) 
                             or (n.obj in (select activity_sid from v_osi_assoc_fle_act where file_sid in (select that_file from v_osi_assoc_fle_fle where this_file=p_obj))))         
        ...
        loop
            ...
            if (v_cnt = 1) then
              ...
              v_tempstring2 := c_block_half_inch || v_cnt || '\tab ';

            else

              v_tempstring2 := '\par\par ' || v_cnt || '\tab ';

            end if;

            v_tempstring := '{\field\fldedit{\*\fldinst HYPERLINK "' || v_url || k.obj || '" }';
            v_tempstring := v_tempstring || '{\fldrslt'; 
            v_tempstring := v_tempstring || '\cs16\ul\cf2 ' || osi_report.replace_special_chars_clob(k.parentinfo, 'RTF') || '}} \par ';

            osi_util.aitc(v_notes, v_tempstring2 );
            osi_util.aitc(v_notes, v_tempstring);
            osi_util.aitc(v_notes, k.note_text);
