...
******************************************************************************/
  ...
  FUNCTION DesktopSQL(FILTER IN VARCHAR2, 
                      ...
                      p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2;
...
******************************************************************************/
    ...
    FUNCTION AddApexSearchFilters(p_OtherSearchCriteria in VARCHAR2, column_names in assoc_arr, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
...
--      ColumnCount NUMBER;
    ...
    BEGIN
         log_error('>>>AddApexSearchFilters(' || p_OtherSearchCriteria || ',column_names' || ',' || p_WorksheetID  || ',' || p_APP_USER  || ',' || p_Instance  || ')');
...
/*    
         for a in (select * from table(split(p_OtherSearchCriteria, '~^~')))
         loop
             log_error('a.column_value=' || a.column_value);
               
             IF instr(a.column_value, '^~^') > 0 THEN
                 
               IF instr(a.column_value, 'Row text contains ') > 0 THEN
                   
                 RowTextContains := replace(replace(a.column_value,'Row text contains ^~^',''), chr(39), chr(39) || chr(39));
               
               ELSE
                 
                 strPOS := instr(a.column_value, '^~^');
                 
                 SQLString := SQLString || vCRLF ||
                              '     AND (upper(' || column_names(substr(a.column_value,1,strPOS-1)) || ') like upper(''%' || replace(substr(a.column_value,strPOS+3), chr(39), chr(39) || chr(39)) || '%' || '''' || '))';
                              
               END IF;
               
             ELSIF instr(a.column_value, 'gReport.column.filter_delete(') > 0 THEN
                    
                  ConditionID := replace(a.column_value, 'gReport.column.filter_delete(' || chr(39), '');
                  ConditionID := replace(ConditionID, chr(39) || ')', '');
                  log_error('ConditionID=' || ConditionID);
                    
                  BEGIN
                       select name,column_name,operator,expr into RowTextContains,ColumnName,Operator,EXPR from apex_030200.wwv_flow_worksheet_conditions where id=conditionid and enabled='Y';
                       ...
                       ELSIF instr(Operator,'contains') > 0 THEN
                            ...
                            SQLString := SQLString || vCRLF ||
                                         '     AND (upper(' || column_names(ColumnName) || ') like upper(''%' || replace(EXPR, chr(39), chr(39) || chr(39)) || '%' || '''' || '))';
                       
                       ELSIF length(Operator) > 0 and length(EXPR) > 0 and length(ColumnName) > 0 then
                       
                            SQLString := SQLString || vCRLF ||
                                         '     AND (' || column_names(ColumnName) || ' ' || Operator || ' ' || '''' || replace(EXPR, chr(39), chr(39) || chr(39)) || '''' || ')';
             ...
             log_error('RowTextContains=' || RowTextContains);
             IF RowTextContains is not null THEN
...
*/         
    ...
    /***************************/ 
    FUNCTION DesktopCFundExpensesSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    BEGIN
         ...
         --- Add Search Filters ---
         SQLString := SQLString || AddApexSearchFilters(p_OtherSearchCriteria, column_names, p_WorksheetID, p_APP_USER, p_Instance);
    ...
    /***************************/ 
    FUNCTION DesktopCFundAdvancesSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /**************************/ 
    FUNCTION DesktopNotificationsSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /***************************************/ 
    FUNCTION DesktopEvidenceManagementSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /*  Activities Section */   
    /***********************/ 
    FUNCTION DesktopActivitiesSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    BEGIN
         ...
         column_names('C005'):='Osi_Object.get_lead_agent_name(a.SID)';
         column_names('C006'):='decode(decode(decode(a.close_date,null,null,''Closed''),null,to_char(A.complete_DATE),''Closed''),null,''Open'',''Closed'',''Closed'',''Completed'')';
         column_names('C007'):='Osi_Unit.get_name(Osi_Object.get_assigned_unit(a.SID))';
         column_names('TO_DATE(C008)'):='o.create_on';
         
         log_error('>>>OSI_DESKTOP.DesktopActivitiesSQL(' || FILTER || ',' || user_sid || ',' || ACTIVE_FILTER || ',' || p_OtherSearchCriteria || ',' || APXWS_MAX_ROW_CNT || ',' || p_WorksheetID || ',' || p_APP_USER || ',' || p_Instance || ')');
         UnitSID := Osi_Personnel.get_current_unit(user_sid);
         
         --- Main Select ---
         SQLString := 'SELECT /*+ INDEX(a PK_OSI_ACT) INDEX(o PK_CORE_OBJ)  INDEX(ot PK_CORE_OBJTYP) */   ''javascript:getObjURL(''''''|| a.SID ||'''''');'' AS url,' || vCRLF ||
                      '       a.ID AS "ID",' || vCRLF ||
                      '       ot.description AS "Activity Type",' || vCRLF ||
                      '       a.title AS "Title",' || vCRLF ||
                      '       Osi_Object.get_lead_agent_name(a.SID) AS "Lead Agent",' || vCRLF ||
                      '       decode(decode(decode(a.close_date,null,null,''Closed''),null,to_char(A.complete_DATE),''Closed''),null,''Open'',''Closed'',''Closed'',''Completed'') AS "Status",' || vCRLF ||
         ...
         --- Add VLT Link ---
         SQLString := SQLString || vCRLF ||
                      '       ''javascript:newWindow({page:5550,clear_cache:''''5550'''',name:''''VLT''||a.sid||'''''',item_names:''''P0_OBJ'''',item_values:''||''''||''''''''||a.sid||''''''''||''''||'',request:''''OPEN''''})'' AS VLT,';
         ...
         CASE 
                              ...
                              WHEN FILTER='ME' THEN
                                                   SQLString := SQLString || vCRLF || 
                                                                '    AND o.sid in (select obj from t_osi_assignment where end_date is null and personnel=''' || user_sid || ''')' || vCRLF ||
                                                                '    ORDER BY ot.description';
             
                            WHEN FILTER='UNIT' THEN
                                                   SQLString := SQLString || vCRLF || 
                                                                '    AND a.assigned_unit=''' || UnitSID ||  '''' || vCRLF ||
                                                                '    ORDER BY ot.description';

                        WHEN FILTER='SUB_UNIT' THEN
                                                   SQLString := SQLString || vCRLF || 
                                                                '    AND a.assigned_unit in ' || Get_Subordinate_Units(UnitSID) || vCRLF ||
                                                                '    ORDER BY ot.description'; 
                                                                            
                        WHEN FILTER='SUP_UNIT' THEN
                                                   SQLString := SQLString || vCRLF || 
                                                                '    AND a.assigned_unit IN ' || Get_Supported_Units(UnitSID) || vCRLF ||
                                                                '    ORDER BY ot.description';
    ...
    /*  Files Section */   
    /******************/ 
    FUNCTION DesktopFilesSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_ObjType IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /*  Participant Section */   
    /************************/ 
    FUNCTION DesktopParticipantSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_ObjType IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /*  Personnel Section */   
    /**********************/ 
    FUNCTION DesktopPersonnelSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    BEGIN
         ...
         column_names('C004'):='osi_unit.get_name(osi_personnel.get_current_unit(o.sid))';
    ...
    /*  Sources Section */   
    /********************/ 
    FUNCTION DesktopSourcesSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /*******************************/ 
    FUNCTION DesktopMilitaryLocationsSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, p_ReturnPageItemName IN VARCHAR2:='', ACTIVE_FILTER IN VARCHAR2) RETURN VARCHAR2 IS
    ...
    /********************************/ 
    FUNCTION DesktopCityStateCountrySQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, p_ReturnPageItemName IN VARCHAR2:='', ACTIVE_FILTER IN VARCHAR2) RETURN VARCHAR2 IS
    ...
    BEGIN
         ...
         IF ACTIVE_FILTER = 'NONE' THEN
           ...
           SQLString := SQLString || vCRLF ||
                        'and contains(o.doc1, nvl(''' || SearchCriteria || ''',''zzz''), 2)>0';
   ...
   END DesktopFullTextSearchSQL;
    ...
    /*  Unit Section */   
    /*****************/ 
    FUNCTION DesktopUnitSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    /*  Workhours Section  */   
    /***********************/ 
    FUNCTION DesktopWorkHoursSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, ACTIVE_FILTER IN VARCHAR2, p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN NUMBER := 10000, p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
   ...
   FUNCTION DesktopSQL(FILTER IN VARCHAR2, user_sid IN VARCHAR2, p_ObjType IN VARCHAR2, p_ReturnPageItemName IN VARCHAR2:='', ACTIVE_FILTER IN VARCHAR2 := 'ACTIVE', NUM_ROWS IN NUMBER := 15, PAGE_ID IN VARCHAR2 := 'P', p_OtherSearchCriteria IN VARCHAR2 := '', APXWS_MAX_ROW_CNT IN VARCHAR2 := '10000', p_WorksheetID IN VARCHAR2 := NULL, p_APP_USER IN VARCHAR2 := NULL, p_Instance IN VARCHAR2 := NULL) RETURN VARCHAR2 IS
    ...
    BEGIN
         log_error('>>>OSI_DESKTOP.DesktopSQL(' || FILTER || ',' || user_sid || ',' || p_ObjType || ',' || p_ReturnPageItemName || ',' || ACTIVE_FILTER || ',' || NUM_ROWS || ',' || PAGE_ID || ',' || p_OtherSearchCriteria || ',' || APXWS_MAX_ROW_CNT || ',' || p_WorksheetID || ',' || p_APP_USER || ',' || p_Instance || ')');
         ...
         CASE 
             WHEN p_ObjType='ACT' THEN
                 ...
                 SQLString := DesktopActivitiesSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
             ...
             WHEN p_ObjType IN ('FILE', 'FILE.INV', 'FILE.INV.CASE', 'FILE.INV.DEV', 'FILE.INV.INFO',
                 ...
                 SQLString := DesktopFilesSQL(NewFilter, user_sid, NewActiveFilter, p_ObjType, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);

             WHEN p_ObjType IN ('PARTICIPANT','PART.INDIV','PART.NONINDIV.COMP','PART.NONINDIV.ORG','PART.NONINDIV.PROG') THEN
        
                 SQLString := DesktopParticipantSQL(NewFilter, user_sid, NewActiveFilter, p_ObjType, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
             
             WHEN p_ObjType='EMM' THEN

                 SQLString := DesktopEvidenceManagementSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
             ...
             WHEN p_ObjType='CFUNDS_ADV' THEN
                 ...
                 SQLString := DesktopCFundAdvancesSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
                 
             WHEN p_ObjType='CFUNDS_EXP' THEN
        
                 SQLString := DesktopCFundExpensesSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);

             WHEN p_ObjType='NOTIFICATIONS' THEN

                 SQLString := DesktopNotificationsSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
             
             WHEN p_ObjType='PERSONNEL' THEN

                 SQLString := DesktopPersonnelSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
             
             WHEN p_ObjType='SOURCES' THEN
        
                 SQLString := DesktopSourcesSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);

             WHEN p_ObjType='MILITARY_LOCS' THEN
                              
                 SQLString := DesktopMilitaryLocationsSQL(NewFilter, user_sid, p_ReturnPageItemName, NewActiveFilter);
             
             WHEN p_ObjType='CITY_STATE_COUNTRY' THEN

                 SQLString := DesktopCityStateCountrySQL(NewFilter, user_sid, p_ReturnPageItemName, NewActiveFilter);

             WHEN p_ObjType='UNITS' THEN

                 SQLString := DesktopUnitSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);

             WHEN p_ObjType='WORKHOURS' THEN

                 SQLString := DesktopWorkHoursSQL(NewFilter, user_sid, NewActiveFilter, p_OtherSearchCriteria, v_max_num_rows, p_WorksheetID, p_APP_USER, p_Instance);
