...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   11:48 Thursday March 22, 2012
--   Exported By:     TWARD
--   Flashback:       0
...
'  end if;'||chr(10)||
'  '||chr(10)||
'  for a in (select * from t';

p:=p||'able(split(apex_application.g_x10,''~'')))'||chr(10)||
'  loop'||chr(10)||
...
p:=p||'LECTION.CREATE_COLLECTION_FROM_QUERY('||chr(10)||
'      p_collection_name=>apex_application.g_x01,'||chr(10)||
'      p_query => OSI_DESKTOP.DesktopSQL(apex_application.g_x02, '||chr(10)||
'                                        :user_sid, '||chr(10)||
...
p:=p||'                                       apex_application.g_x07, '||chr(10)||
