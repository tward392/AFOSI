INSERT INTO T_HTML_TEMPLATE ( NAME, DT, FLOWERBOX, MODIFY_BY, MODIFY_ON ) VALUES ( 'cfunds_expenses_for_indiv',  TO_Date( '09/21/2011 09:00:06 AM', 'MM/DD/YYYY HH:MI:SS AM'), NULL, 'HQCUIFW0334',  TO_Date( '09/21/2011 09:00:04 AM', 'MM/DD/YYYY HH:MI:SS AM')); 
COMMIT;
...
-- angela change - merge.
  ...
  08-Sep-11 TJW  CR#3916 - Inactive Paragraphs showing in CFMS.
                  Changed in expense_details (added ACTIVE='Y' or PARAGRAPH=PPARGRAPH to the where clause).
  30-Sep-11 JDB  Added 5 changes to the form 29 for end of year
  04-Oct-11 TJW  CR#3930 - Add Activity Link to the CFMS Expenses Page.
                  Changed Context in expense_details to do a Link.
  04-Oct-11 TJW  CR#3921 - Add Paragraph to Expenses List.
                  Changed expenses_for_indiv.
*/
...
--                     || '<tr><td width="70"> </td><td colspan="3" width="320"><font color="red">* Warning: It is possible that this amount could change prior to when the actual "Pay Expense" button is clicked.</font></td></tr>';
            ...
            v_ok := Web_Template_Pkg.replace_tag(v_template, 'CONTEXT', '<A target="_new" HREF="' || core_util.get_config('CORE.BASE_URL') || e.PARENT || '"' || ' Title="Open in I2MS" ' || '>' || core_obj.get_parentinfo(e.parent) || '</A> <BR>');
...
--                OWA_CORE_LISTPRINT(V_CURSOR2, 'pCategory', 1, FALSE);
            ...
            ELSIF v_tag = 'PARAGRAPH' THEN
                ...
                */
                ...
                v_query2 := v_query2 || '     from T_CFUNDS_PARAGRAPHS where active=''Y'' or paragraph=' || '''' || pparagraph || '''' || ') cfp';
                DBMS_SQL.PARSE(v_cursor2, v_query2, dbms_sql.native);
    ...
    BEGIN
        ...
        LOOP
            ...
            IF v_tag = 'EXPENSE_LIST' THEN
                ...
                v_query := v_query || ',e.paragraph_number || ''' || '-' || ''' || e.paragraph_content';
                v_query := v_query || ' from V_CFUNDS_EXPENSE_V3 e';
                v_query := v_query || ' where ';
                v_query :=
                    v_query || '     Cfunds_Pkg.Get_Fiscal_Year(e.INCURRED_DATE) = '''
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF v_usage = 'a' THEN
                    ...
                    v_ok := Web_Template_Pkg.replace_tag(v_template, 'TITLE', 'E-FUNDS AUDIT');
                    v_ok := Web_Template_Pkg.replace_tag(v_template, 'HEADING', 'E-FUNDS AUDIT');
                ...
                ELSIF v_usage = 'r' THEN
                    -- put the appropriate headers on the form.
                    v_ok := Web_Template_Pkg.replace_tag(v_template, 'TITLE', 'E-FUNDS ACCOUNT DETAILS');
                    v_ok := Web_Template_Pkg.replace_tag(v_template, 'HEADING', 'E-FUNDS ACCOUNT DETAILS');
                    -- hide this section when the form is used as an audit form.
                    -- these tags are at the bottom of the html template in a script.
                    v_ok :=
                        Web_Template_Pkg.replace_tag(v_template,
                ...
                LOOP
                    ...
                    ELSIF v_tag = 'SUBORDINATE_UNITS_LIST' THEN
                        ...
                        -- only show active E-Funds Units that are subordinate.
