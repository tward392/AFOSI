...
/******************************************************************************
   ...
   Revisions:
    ...
    22-Jul-2011  Tim Ward       CR#3880 - ROI Narrative Sort Order issues.
                                  Changed in initialize_activities.
...
******************************************************************************/
    ...
    procedure initialize_activities(p_spec in varchar2 := null, p_obj in varchar2, p_report_type in varchar2) is
    ...
    begin
         if p_spec is null then
           ...
           v_spec := osi_report_spec.create_instance(core_obj.lookup_objtype('ALL.REPORT_SPEC'), p_obj, p_report_type);
         ...
         select count(*) into v_new from V_OSI_RPT_AVAIL_ACTIVITY where spec=v_spec;
         ...
         if core_obj.get_objtype(p_obj) = core_obj.lookup_objtype('FILE.SOURCE') then
           for a in(select sid from t_osi_activity 
                       where (source = p_obj) --ME filter
                         ...
                         and sid not in(select activity 
                                         ...
                                         where spec = v_spec))
           loop
               select max(seq) + 1 into v_seq from t_osi_report_activity where spec = v_spec;
           ...
           for a in(select activity_sid from t_osi_assoc_fle_act
