UPDATE T_CFUNDS_RECEIPT_DISPOSITION SET DESCRIPTION=REPLACE(DESCRIPTION,'$75','$100') 
  WHERE DESCRIPTION LIKE '%$75%';
COMMIT;

UPDATE T_CFUNDS_PARAGRAPHS SET ACTIVE='N'
  WHERE CONTENT IN ('Formal Liaison Functions.','Informal Liaison Functions.');
COMMIT;


alter table t_cfunds_paragraphs add seq number;

update t_cfunds_paragraphs set seq=to_number(replace(replace(replace(substr(paragraph_number,2),'.',''),'A',''),'J',''));
commit;

update t_cfunds_paragraphs set seq=999999 where paragraph_number='SJ';
commit;
...
--   Application:     100
--   Name:            Web-I2MS
--   Date and Time:   10:34 Monday August 22, 2011
--   Exported By:     TWARD
--   Flashback:       0
...
'',
  ...
  p_last_upd_yyyymmddhh24miss => '20110822103358',
...
'                    in Numerical Order and not ASCII order.');
