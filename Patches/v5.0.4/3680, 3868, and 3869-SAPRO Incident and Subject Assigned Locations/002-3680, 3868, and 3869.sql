...
---------------------------------------------------
---------------------------------------------------
--- Add SAPRO Fields to T_OSI_F_INV_SPEC        ---
--- Add SAPRO Field to T_OSI_PARITICIPANT_HUMAN ---
--- Add SAPRO Field to V_OSI_PARTIC_SA          ---
--- Replace OSI_DESKTOP Spec and Body BOTH      ---
---------------------------------------------------
---------------------------------------------------
---------------------------------------------------
alter table t_osi_f_inv_spec add SAPRO_INCIDENT_LOCATION_CODE VARCHAR2(10);

ALTER TABLE WEBI2MS.t_osi_f_inv_spec ADD (
  CONSTRAINT FK_SAPRO_LOCATION_CODE 
 FOREIGN KEY (SAPRO_INCIDENT_LOCATION_CODE ) 
 REFERENCES WEBI2MS.T_SAPRO_LOCATIONS(LOCATION_CODE));



alter table t_osi_f_inv_spec add SAPRO_INCIDENT_LOCATION_CSC VARCHAR2(20);

ALTER TABLE WEBI2MS.t_osi_f_inv_spec ADD (
  CONSTRAINT FK_SAPRO_INCIDENT_LOCATION_CSC 
 FOREIGN KEY ( SAPRO_INCIDENT_LOCATION_CSC ) 
 REFERENCES WEBI2MS.T_SAPRO_CITY_STATE_COUNTRY(SID));


ALTER TABLE T_OSI_PARTICIPANT_HUMAN ADD SAPRO_ASSIGNED_LOCATION_CODE VARCHAR2(10);

ALTER TABLE WEBI2MS.T_OSI_PARTICIPANT_HUMAN ADD (
  CONSTRAINT FK_SAPRO_ASS_LOCATION_CODE 
 FOREIGN KEY (SAPRO_ASSIGNED_LOCATION_CODE ) 
 REFERENCES WEBI2MS.T_SAPRO_LOCATIONS(LOCATION_CODE));
...
--                      '       ''<a href="#" style="text-decoration:none; cursor:default;" title="Activity: '' || osi_activity.get_id(e.parent) || '' - '' || core_obj.get_tagline(e.parent) || ''">'' || substr(''Activity: '' || osi_activity.get_id(e.parent) || '' - '' || core_obj.get_tagline(e.parent),1,25) || case when length(''Activity: '' || osi_activity.get_id(e.parent) || '' - '' || core_obj.get_tagline(e.parent)) > 25 then ''...'' end || ''</A>'' AS "Context",' || vCRLF ||
                      '       TO_CHAR(e.total_amount_us, ''FML999G999G999G990D00'') AS "Total Amount",' || vCRLF ||
                      '       e.description AS "Description",' || vCRLF ||
--                      '       ''<a href="#" style="text-decoration:none; cursor:default;" title="'' || substr(e.description,1,3900) || ''">'' || substr(e.description,1,25) || case when length(e.description) > 25 then ''...'' end || ''</A>'' AS "Description",' || vCRLF ||
