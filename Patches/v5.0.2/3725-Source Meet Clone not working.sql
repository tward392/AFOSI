...
/******************************************************************************
   ...
   Revisions:
    ...
    02-Mar-2011  Tim Ward        CR#3725 - Added code to the clone function so the clone will work.
******************************************************************************/
    ...
    procedure clone(p_obj in varchar2, p_new_sid in varchar2) is
        v_osi_a_source_meet_record   t_osi_a_source_meet%rowtype;
    begin
        --Get Activity record
        select *
          into v_osi_a_source_meet_record
          from t_osi_a_source_meet
         where sid = p_obj;

        --Insert a new  Activity record
        insert into t_osi_a_source_meet
                    (sid, contact_method,next_meet_date,commodity,info_received,info_levied)
             values (p_new_sid,
                     v_osi_a_source_meet_record.contact_method,
                     v_osi_a_source_meet_record.next_meet_date,
                     v_osi_a_source_meet_record.commodity,
                     v_osi_a_source_meet_record.info_received,
                     v_osi_a_source_meet_record.info_levied);
    exception
        when others then
            log_error('osi_source_meet.clone: ' || sqlerrm);
            raise;
    end clone;
    ...
    begin
          ...
          case
                when v_status_code = 'AA' then
                      ...
                      update t_osi_f_poly_file set requesting_unit = core_util.get_config('POLY_SID')
                            where sid = p_obj;
                when v_status_code = 'AC' then
                      --transfer ownership to Poly Auth Unit
                      transfer_file(p_obj, core_util.get_config('POLY_SID'), v_transfer_reason);
                      update t_osi_f_poly_file set rpo_unit = core_util.get_config('POLY_SID')
                            where sid = p_obj;
          ...
          /* if status is reverting to 'New' or 'Open' transfer ownership to PREVIOUS unit. */ 
                when v_status_code in('NW','OP') then
                      ...
                      /* iterate through obj ownership records - 1 (current) 
                               and return the prev unit */
                      for a in(select unit_sid
                                 from t_osi_f_unit
                                where file_sid = p_obj
                                  and rownum = v_owner_cnt - 1
                                order by start_date)
                      loop
