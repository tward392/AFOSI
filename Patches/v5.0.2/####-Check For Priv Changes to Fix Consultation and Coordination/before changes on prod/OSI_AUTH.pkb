CREATE OR REPLACE PACKAGE BODY WEBI2MS.Osi_Auth AS
/******************************************************************************
   Name:     osi_auth
...
--###################################################################################################
    ...
    FUNCTION check_for_priv(
        ...
        function check_action_explicit(p_obj_type in varchar2, p_action in varchar2)
        return varchar2 is
        begin
         --*****************************************************************
         --Returns:
         ---Y means a priv existed and it passed
         ---N means a priv existed and did not pass
         ---X means a priv did not exist
         --*****************************************************************
               
        --First, check to see if their is an action for the exact object type
        for i in (select 1
                    from t_osi_auth_priv oap, t_osi_auth_action_type oaat
                   where oap.action = oaat.sid and oap.obj_type = p_obj_type and oaat.code = v_action)
        loop
            --If so: see if the personnel has that priv
            --Check Role
            for j in (select 1
                       ...
                       where ur.personnel = v_personnel
                         ...
                         and rp.priv = p.sid
                         and at.sid = p.action
                         and at.code = v_action
                         and p.obj_type = p_obj_type
                         and ur.enabled = 'Y'
                         and rp.enabled = 'Y'
            ...
            --Check Priv
            for j in (select 1
                        from t_osi_personnel_priv pp, t_osi_auth_priv p, t_osi_auth_action_type at
                       where pp.personnel = v_personnel
                         ...
                         and (   pp.unit is null
                              or pp.unit = v_unit
                              or (    pp.include_subords = 'Y'
                                  and osi_unit.is_subordinate(pp.unit, v_unit) = 'Y'))
                         and pp.enabled = 'Y'
                         and sysdate between nvl(pp.start_date, sysdate - 1)
                                         and nvl(pp.end_date, sysdate + 1))
            loop
                return 'Y';
            end loop;
            ...
            --If priv not found, return FAILURE
            return 'N';
        end loop;
        
        return 'X';
        
        end check_action_explicit;
        
        function get_base_obj_type_code(p_obj_type_code in varchar2) return varchar2
        is
            v_return varchar2(200);
            v_chr number;
        begin
            --Reverse String
            select reverse(p_obj_type_code) into v_return from dual;
            --Find first '.'
            v_chr := instr(v_return, '.');
            --Remove last section
            v_return := substr(v_return, v_chr + 1, length(v_return) - v_chr);
            --Reverse again
            select reverse(v_return)
                into v_return
                from dual;
            --Return result
            return v_return;
        end get_base_obj_type_code;
    BEGIN
        ...
        --See if we pass on an explicit obj type tied priv [XXX.XXX.XXX (if exists, XXX.XXX if not)]
        if (check_action_explicit(p_obj_type, v_action) = 'Y') then
        
            return 'Y';
            
        else
            --Get the obj type code
            v_obj_type := osi_object.GET_OBJTYPE_CODE(p_obj_type);
            
            --Get the base of the obj_type_code
            if freq_instr(v_obj_type,'.') > 1 then
                v_obj_type := get_base_obj_type_code(v_obj_type);
            end if;

            --Now see if we pass on a parent level obj type [XXX.XXX where XXX.XXX.XXX exists]
            --First, see if there is a parent level obj code anyway (if not, we will be left with ACT, FLE, etc.) - otherwise you will have something like ACT.AAPP
             if (instr(v_obj_type, '.') >0) then
                -->See if a priv for this 
                --First need to see if there is an object type for this object type code
                for k in (select sid from t_core_obj_type where code = v_obj_type)
                loop
                   --The object type existed so run a check on it
                     v_temp := check_action_explicit(k.sid, v_action);
                     --Returns:
                     ---Y means a priv existed and it passed
                     ---N means a priv existed and did not pass
                     ---X means a priv did not exist (in which case we will continue to check inherited privs)
                     if (v_temp = 'Y') then
                        return 'Y';
                     elsif (v_temp = 'N') then
                        return 'N';
 ...
 END LogCheckAccessReturn;
    ...
    BEGIN
         ...
         IF NOT(check_for_priv('OVERRIDE', v_obj_type, v_personnel, v_obj_unit, p_supress_logging => true) = 'Y' OR v_assigned = 'Y') THEN
         ...
         IF myRtn = 'Y' THEN
           ...
           IF check_for_priv('ACCESS', v_obj_type, v_personnel, p_supress_logging => true) = 'N' THEN       

             FailMsg := 'check_for_priv(ACCESS,' || v_obj_type || ',' || v_personnel || ')';
             myRtn := 'N';

           ELSE

             FailMsg := 'check_for_priv(ACCESS,' || v_obj_type || ',' || v_personnel || ')';--NULL;
             myRtn := 'Y';
           ...
           if v_obj_unit != '<none>' then --Only do the following checks if Unit Ownership is applicable
             ...
             IF myRtn = 'N' and check_for_priv('ACCESS_UNT', v_obj_type, v_personnel, v_obj_unit, p_supress_logging => true) = 'N' THEN

               FailMsg := 'check_for_priv(ACCESS_UNT,' || v_obj_type || ',' || v_personnel || ',' || v_obj_unit || ')';
               myRtn := 'N';

             ELSE

               FailMsg := 'check_for_priv(ACCESS_UNT,' || v_obj_type || ',' || v_personnel || ',' || v_obj_unit || ')';--NULL;
               myRtn := 'Y';
             ...
             if (myrtn = 'N') then
               ...
               if (v_assigned = 'Y' and check_for_priv('ACCESS_ASS', v_obj_type, v_personnel, p_supress_logging => true) = 'Y') then

                 FailMsg := 'check_for_priv(ACCESS_ASS,' || v_obj_type || ',' || v_personnel || ')';--NULL;
                 myRtn := 'Y';

               else
 
                 FailMsg := 'check_for_priv(ACCESS_ASS,' || v_obj_type || ',' || v_personnel || ')';
                 myRtn := 'N';
